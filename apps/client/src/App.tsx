...
export function App() {
  ...
  const [viewState, setViewState] = useState<ViewState>(() => readViewStateFromUrl());
  const [selectedColor, setSelectedColor] = useState<DrawingColor>(() => readStoredColor());
  const [selectedBrushSize, setSelectedBrushSize] = useState<BrushSize>(() => readStoredBrushSize());
  const canvasHostRef = useRef<HTMLDivElement | null>(null);
  const viewStateRef = useRef(viewState);
  const flushUrlRef = useRef<() => void>(() => undefined);
  const socketRef = useRef<WebSocket | undefined>(undefined);
  const toolRef = useRef({ color: selectedColor, brushSize: selectedBrushSize });
  ...
  const eventQueueRef = useRef<
    Array<
      Extract<
        ServerEvent,
        { type: "drawing_committed" | "stroke_started" | "stroke_updated" | "stroke_cancelled" }
      >
    >
  >([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/bootstrap", {
      ...
      .then((payload) => {
        ...
        window.localStorage.setItem(revisionStorageKey, String(payload.revision));
      })
      .catch((error: unknown) => {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      socket.addEventListener("open", () => {
        reconnectAttempt = 0;
        const lastSeenRevision = Number(window.localStorage.getItem(revisionStorageKey) ?? "0");
        socket?.send(
          JSON.stringify({
            type: "subscribe",
      ...
      socket.addEventListener("message", (event) => {
        ...
        if (serverEvent.type === "subscribed") {
          ...
          window.localStorage.setItem(revisionStorageKey, String(serverEvent.revision));
        ...
        if ("revision" in serverEvent) {
          window.localStorage.setItem(revisionStorageKey, String(serverEvent.revision));
          setBootstrap((current) =>
            current.status === "ready" ? { ...current, revision: serverEvent.revision } : current
          );
        }
      });
  ...
  useEffect(() => {
    ...
    const transientStrokes = new Map<string, Pick<Drawing, "path" | "color" | "brushSize">>();
    const pendingLocalStrokes = new Map<string, Pick<Drawing, "path" | "color" | "brushSize">>();
    scene.add(sphere);
    ...
    const faceUrls = cubeFaceNames.map((faceName) => bootstrap.snapshotFaces[faceName]?.url);
    ...
    const rotateToPointerPoint = (anchorPoint: UnitVector, pointerPoint: UnitVector, elapsedMs: number) => {
    ...
    const onPointerDown = (event: PointerEvent) => {
      ...
      const shouldRotate = !isDrawingZoom(viewStateRef.current.zoom) || touches.size >= 2;

      if (!shouldRotate) {
        stopInertia(inertiaVelocity, releaseVelocity);
        wheelInertia.deltaX = 0;
        wheelInertia.deltaY = 0;
        const point = spherePointFromPointer(event, renderer, camera, sphere, raycaster, pointerNdc);
        if (point) {
      ...
      if (activeStroke) {
        ...
        repaintOverlay(transientOverlay, transientStrokes.values());
        sendClientMessage({ type: "stroke_cancelled", strokeId: activeStroke.id });
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      if (!pointer.active || !pointer.rotating || (pointer.id !== event.pointerId && touches.size < 2)) {
        if (pointer.active && pointer.drawing && pointer.id === event.pointerId && activeStroke) {
          const point = spherePointFromPointer(event, renderer, camera, sphere, raycaster, pointerNdc);
          if (point && shouldStorePathPoint(activeStroke.path.at(-1), point, activeStroke.brushSize)) {
      ...
      const rotationPoint = spherePointFromPointer(event, renderer, camera, sphere, raycaster, pointerNdc);

      if (pointer.anchorPoint && rotationPoint) {
        rotateToPointerPoint(pointer.anchorPoint, rotationPoint, now - pointer.lastAt);
      }
      pointer.lastX = event.clientX;
      pointer.lastY = event.clientY;
      pointer.lastAt = now;
    ...
    const onPointerUp = (event: PointerEvent) => {
      ...
      if (pointer.id === event.pointerId || (pointer.rotating && pointer.multiTouch && touches.size < 2)) {
        const wasRotating = pointer.rotating;
        if (pointer.drawing && activeStroke) {
          let keepPendingStroke = false;
          if (activeStroke.path.length > 0 && connectionStateRef.current === "connected") {
            sendClientMessage({
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      const nextWheelDeltaX = wheelInertia.deltaX * wheelInertiaAccumulation + event.deltaX * wheelInertiaBoost;
      const nextWheelDeltaY = wheelInertia.deltaY * wheelInertiaAccumulation + event.deltaY * wheelInertiaBoost;
      const nextWheelMagnitude = Math.hypot(nextWheelDeltaX, nextWheelDeltaY);

      if (nextWheelMagnitude >= startWheelInertiaDelta) {
        const clampedScale = Math.min(1, maxWheelInertiaDelta / nextWheelMagnitude);
    ...
    const animate = () => {
      ...
      if (pendingMultiTouchRotation && pointer.active && pointer.rotating && touches.size >= 2) {
        const now = performance.now();
      ...
      for (const event of eventQueueRef.current.splice(0)) {
        if (event.type === "drawing_committed") {
          const pendingEntry = findPendingStroke(pendingLocalStrokes, event.drawing.path);
          if (pendingEntry) {
        ...
        if (event.type === "stroke_started") {
          ...
          transientStrokes.set(key, { path: [event.point], color: event.color, brushSize: event.brushSize });
          repaintOverlay(transientOverlay, transientStrokes.values());
        }
        ...
        if (event.type === "stroke_cancelled") {
          ...
          for (const [key] of transientStrokes) {
            if (event.strokeId === "*" ? key.startsWith(`${event.connectionId}:`) : key === `${event.connectionId}:${event.strokeId}`) {
      ...
      if (current.autoRotateEnabled && !isDrawingZoom(current.zoom)) {
        ...
        const orientation = quaternionStateToThree(current.orientation).multiply(delta).normalize();
        updateView(
          {
            ...current,
            orientation: quaternionToState(orientation)
  ...
  const updateViewFromControls = (updater: (current: ViewState) => ViewState) => {
  ...
  return (
    <main className="app-shell">
      <section className="globe-stage" aria-label="Shared drawing sphere">
        <div ref={canvasHostRef} className={`sphere-host ${drawingMode ? "drawing-cursor" : "rotation-cursor"}`} />
      </section>

      <header className="top-overlay" aria-label="World status">
        <div className="overlay-cluster brand-cluster">
          <span>
        ...
        <div className="overlay-cluster action-cluster">
          <button
            type="button"
            className={viewState.autoRotateEnabled ? "selected" : undefined}
            onClick={() =>
          ...
          >
            {viewState.autoRotateEnabled ? (autoRotateActive ? "Auto-rotate on" : "Auto suspended") : "Auto-rotate off"}
          </button>
        </div>
      </header>

      <aside className="bottom-overlay" aria-label="Drawing controls">
        ...
        {drawingMode ? (
          <>
            <div className="palette" aria-label="Drawing palette">
            ...
            <div className="control-row">
              {brushSizes.map((brushSize) => (
                <button
                  key={brushSize}
                  type="button"
                  className={selectedBrushSize === brushSize ? "selected" : undefined}
                  disabled={connectionState !== "connected"}
        ...
        {bootstrap.status === "error" ? <p className="error">{bootstrap.message}</p> : null}
      </aside>
    </main>
  );
}
...
function readViewStateFromUrl(): ViewState {
  ...
  const zoom = clampZoom(readFiniteNumber(params.get("z"), viewSettings.initialZoom));
...
function writeViewStateToUrl(viewState: ViewState) {
  ...
  if (viewState.autoRotateEnabled) {
    ...
    const orientation = normalizeQuaternionState(viewState.orientation) ?? identityQuaternionState();
...
function readOrientationFromUrl(params: URLSearchParams): QuaternionState | undefined {
...
function readStoredColor(): DrawingColor {
  ...
  return drawingPalette.includes(color as DrawingColor) ? (color as DrawingColor) : "#111111";
}

function readStoredBrushSize(): BrushSize {
  const brushSize = window.localStorage.getItem(brushStorageKey);
  return brushSizes.includes(brushSize as BrushSize) ? (brushSize as BrushSize) : "fine";
}
...
function normalizeQuaternionState(orientation: QuaternionState): QuaternionState | undefined {
...
function quaternionStateToThree(orientation: QuaternionState) {
  const normalized = normalizeQuaternionState(orientation) ?? identityQuaternionState();
...
function isIdentityQuaternion(quaternion: THREE.Quaternion) {
  return Math.abs(quaternion.x) < 0.000001 && Math.abs(quaternion.y) < 0.000001 && Math.abs(quaternion.z) < 0.000001 && Math.abs(quaternion.w - 1) < 0.000001;
}
...
function quaternionToAxisAngle(quaternion: THREE.Quaternion) {
  ...
  return {
    axis: new THREE.Vector3(normalized.x / scale, normalized.y / scale, normalized.z / scale).normalize(),
    angle
  };
}
...
function paintDrawingOnOverlay(overlay: OverlayCubeTexture, drawing: Pick<Drawing, "path" | "color" | "brushSize">) {
